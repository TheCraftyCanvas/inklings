...
<div id="page-wrapper"><div id="page">
    ...
    <header class="clearfix<?php print $linked_site_logo ? ' with-logo' : ''; ?>" role="banner">
      <div id="branding">
        <?php if ($site_name || $site_slogan): ?>
          <?php if (!$site_slogan && $hide_site_name): ?> class="<?php print $visibility; ?>"<?php endif; ?>
          <hgroup>
          <img id="philly-citywide-tutoring" src="/<?php print path_to_theme()?>/images/philly-skyline-from-clocktower.png" alt=""/>
          <div id="heading">
            <?php if ($site_name): ?>
  ...
  <?php if ($menubar = render($page['menu_bar'])): ?>
    <div id="menu-bar-wrapper"><div class="container clearfix">
      <div id="desktop-menu"><?php print $menubar; ?></div><!--/#desktop-menu-->
      <div id="mobile-menu" >
      <?php 
        $menuList = theme('links', array('links' => menu_navigation_links('main-menu'))); 
        $menuList = str_replace('><a href','value', $menuList);
        $menuList = str_replace('</a>','', $menuList);
        $menuList = str_replace('class="active"','selected="selected"', $menuList);
        $menuList = str_replace('<li','<option', $menuList);
        $menuList = str_replace('</li>','</option>', $menuList);
        $menuList = str_replace('<ul class="links">','<select id="mobile-nav">',$menuList);
        $menuList = str_replace('</ul>','</select>',$menuList);
        print $menuList;
      ?>
      </div><!--/#mobile-menu -->
    </div></div>
