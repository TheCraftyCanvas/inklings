 ...
 */
function inklings_default_theme_settings() {
  $defaults = array(
    'style' => 'grey',
    'layout-width'    => 0,
    'sidebarslayout'  => 0,
 ...
 */
function inklings_initialize_theme_settings($theme_name) {
  ...
  if (is_null($theme_settings['fix_css_limit']) || $theme_settings['rebuild_registry'] == 1) {
    ...
    $defaults = inklings_default_theme_settings();
...
*   array A form array.
*/
function inklings_settings($saved_settings) {
  global $base_url;
  ...
  $form['tnt_container']['layout_settings']['sidebarslayout'] = array(
    ...
    '#description' => t('<b>Variable width sidebars (wide)</b>: If only one sidebar is enabled, content width is 250px for left sidebar and 300px for right sidebar. If both sidebars are enabled, content width is 160px for left sidebar and 234px for right sidebar. <br /> <b>Fixed width sidebars (wide)</b>: Content width is 160px for left sidebar and 234px for right sidebar. <br /> <em>Equal width sidebars</em> can be customized in _custom/custom-style.css. For other details, please refer to readme.txt.'),
    '#options' => array(
