...
function get_inklings_style() {
  $style = theme_get_setting('style');
  return $style;
}
drupal_add_css(drupal_get_path('theme','inklings').'/css/style-zero.css');
drupal_add_css(drupal_get_path('theme','inklings') . '/css/' . get_inklings_style() . '.css');
drupal_add_css(drupal_get_path('theme','inklings').'/_custom/custom-style.css');

$roundcorners = theme_get_setting('roundcorners');
  if ($roundcorners == '1'){ 
  drupal_add_css(drupal_get_path('theme','inklings').'/css/round.css', 'theme');
}
 ...
 */
function inklings_preprocess(&$vars) {
...
function inklings_preprocess_page(&$vars) {
  global $language;
...
// Use grouped import technique for more than 30 un-aggregated stylesheets (css limit fix for IE)
  ...
  if (isset($_SERVER['HTTP_USER_AGENT']) && (strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') !== false)) {
    if (theme_get_setting('fix_css_limit') && !variable_get('preprocess_css', FALSE) && inklings_css_count($css) > 26) {
      $styles = '';
      $suffix = "\n".'</style>'."\n";
...
function inklings_preprocess_block(&$vars) {
...
function inklings_preprocess_node(&$vars) {
...
function inklings_preprocess_comment(&$vars) {
  global $user;
 ...
 */
function inklings_preprocess_comment_wrapper(&$vars) {
 ...
 */
function inklings_preprocess_views_view(&$vars) {
 ...
 */
function inklings_preprocess_search_result(&$variables) {
 ...
 */
function inklings_username($object) {
 ...
 */
function inklings_file($element) {
 ...
 */
function inklings_css_count($array) {
  $count = 0;
  foreach ($array as $item) {
    $count = (is_array($item)) ? $count + inklings_css_count($item) : $count + 1;
  }
  return $count;
}
 ...
 */
function inklings_themesettings_link($prefix, $suffix, $text, $path, $options) {
 ...
 * Breadcrumb override
 */
function inklings_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
...
$preload = theme_get_setting('cssPreload'); // print the js file if css image preload enabled
  if ($preload == '1'){
    drupal_add_js(drupal_get_path('theme','inklings').'/js/preloadCssImages.jQuery_v5.js'); // load the javascript
...
if($dropdownjs == '1'){ 
  drupal_add_js(drupal_get_path('theme','inklings').'/js/jquery.hoverIntent.minified.js'); // load the javascript
	drupal_add_js(drupal_get_path('theme','inklings').'/js/dropdown.js'); // load the javascript	
}
...
function inklings_submit($element) {
